    ...
    <div className="flex h-[580px] flex-col rounded-2xl border border-slate-200 bg-white shadow-xs overflow-hidden">
      <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/80 px-5 py-3.5">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-600 text-white shadow-md shadow-cyan-500/20">
      ...
      <form onSubmit={handleSubmit} className="border-t border-slate-200 bg-white p-3.5">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="Type your exam question (e.g. Generate a study plan)..."
            disabled={isLoading}
            className="flex-1 rounded-xl border border-slate-300 bg-slate-50 px-3.5 py-2 text-xs text-slate-800 focus:border-cyan-600 focus:bg-white focus:outline-hidden disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-600 text-white shadow-md shadow-cyan-500/20 hover:bg-cyan-700 disabled:opacity-50 transition-all shrink-0 cursor-pointer"
          >
