...
const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  onSendMessage,
  isLoading,
  title = 'LMS AI Mentor Chat',
  subtitle = 'Ask natural language questions about your uploaded Excel data',
}) => {
      ...
      <form onSubmit={handleSubmit} className="border-t border-slate-200 bg-white p-3.5">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="Type your LMS query (e.g. Analyze my weak subjects)..."
            disabled={isLoading}
            className="flex-1 rounded-xl border border-slate-300 bg-slate-50 px-3.5 py-2 text-xs text-slate-800 focus:border-blue-600 focus:bg-white focus:outline-hidden disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-600 text-white shadow-md shadow-blue-500/20 hover:bg-blue-700 disabled:opacity-50 transition-all shrink-0 cursor-pointer"
          >
